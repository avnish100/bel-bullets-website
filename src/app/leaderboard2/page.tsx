  ...
  async function fetchLeaderboardData() {
    ...
    try {
      ...
      const userStatsData = userRank !== -1 ? {
        rank: userRank + 1,
        activity: rankedStats[userRank].activity_count,
  ...
  async function fetchRecentActivities() {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('activities2')
        .select('id, start_date, type, elapsed_time')
        .eq('user_id', userId)
                    ...
                    <TableBody>
                      {topRunners.map((runner) => (
                        <TableRow key={runner.user_id}>
                          ...
                          <TableCell>{runner.activity_count.toFixed(1)}</TableCell>
                ...
                <CardContent>
                  {userStats ? (
                    <div className="text-center">
                      ...
                      <p>Activities: {userStats.activity.toFixed(1)} </p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <CircularProgress progress={((userStats?.activity ?? 1) / monthlyActivityGoal) * 100} size={200}/>
              <p className="text-center text-xl font-semibold">
                {userStats?.activity.toFixed(0)} / {monthlyActivityGoal} 
              </p>
              <p className="text-center text-muted-foreground">
                {((userStats?.activity ?? 1) / monthlyActivityGoal * 100).toFixed(0)}% completed keep pushing !
              </p>
            </div>
          </CardContent>
        </Card>
              ...
              <TableBody>
                {activities.map((activity) => (
                  <TableRow key={activity.id}>
                    ...
                    <TableCell>{formatTime(activity.elapsed_time)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
